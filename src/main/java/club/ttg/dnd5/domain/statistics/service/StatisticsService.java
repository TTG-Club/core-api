...
public class StatisticsService {
    ...
    private final List<Class<?>> COUNTED_ENTITIES = List.of(Spell.class, Species.class, Feat.class, Background.class, MagicItem.class, Item.class, Glossary.class);
    ...
    public Long countAllMaterials() {
        return COUNTED_ENTITIES_TABLE_NAMES.stream().map(
                        tableName -> entityManager.createNativeQuery(String.format("SELECT COUNT(*) FROM %s WHERE is_hidden_entity = false", tableName), Long.class)
                                .getSingleResult())
                .mapToLong(e -> (Long)e)
                .sum();
    }
