...
/**
 * Постоянные модификаторы листа персонажа: то, что черта меняет в шапке и в блоке защит
 * раз и навсегда, без условий и активации.
 *
 * <p>Модель типизированная, а не {@code ActiveEffect} в вокабуляре VTTG, которым описаны
 * магические предметы и заклинания. Причины три: редактор черт получает форму с
 * выпадающими списками вместо строковых ключей вроде {@code movement.walk}; прибавка к
 * максимуму хитов у «Крепкого» зависит от уровня взятия черты и в паре «ключ — значение»
 * не выражается; сопротивление по выбору ссылается на {@code mechanics.choices}. Перевод
 * в вокабуляр VTTG — задача мапперов экспорта, как это уже сделано для вида.</p>
 *
 * <p>Условные эффекты сюда не попадают: «Оборона» даёт +1 к КД только в доспехе,
 * «Дар духа ночи» — сопротивление только в темноте, «Налётчик» ускоряет только на
 * действие Рывок. Условие пришлось бы описывать отдельным языком, а лист всё равно
