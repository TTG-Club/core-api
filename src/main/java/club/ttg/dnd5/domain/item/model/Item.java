...
public class Item extends NamedEntity {
    ...
    /**
     * Тратит ли применение единицу предмета: зелья, яды, боеприпасы, еда
     * ({@code DnDGameItem.consumable} в вокабуляре VTTG). Пусто — не расходуется.
     *
     * <p>Своим полем, а не только выводом по типам предмета: с dnd5e-2024 0.8.62
     * стрелковое оружие на листе заряжают любым расходуемым предметом, и решает это
     * именно признак расхода. Тип его не предсказывает — «Мыло» расходуется, а
     * «Верёвка» нет, — поэтому признак ставит мастерская, как у магического
     * предмета.</p>
     */
