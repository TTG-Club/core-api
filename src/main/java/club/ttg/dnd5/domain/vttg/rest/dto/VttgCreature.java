...
public class VttgCreature {
    ...
    /**
     * Заклинания существа готовыми записями справочника — с зарядами и заклинательной
     * характеристикой того блока, в котором заклинание заведено.
     *
     * <p>Соседом {@link #system}, а не его полем: система читает их как
     * {@code DnDCreature.spells} верхнего уровня. Параметры блоков (Сл, бонус атаки,
     * порции) лежат отдельно — в {@code system.spellcasting}.</p>
     *
     * <p>Одно и то же заклинание в двух порциях отдаётся один раз, по первой порции, где
     * оно встретилось: иначе на листе существа был бы дубль.</p>
     */
    private List<VttgSpell> spells;
}
