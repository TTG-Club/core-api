...
public class VttgFeat {
    ...
    /**
     * Применяемые дары черты — их лист персонажа проставляет сам: владения, защиты,
     * модификаторы, повышение характеристик, выборы при взятии, ресурсы, заклинания и
     * требования. Требование лежит внутри целиком: разобранные поля рядом с
     * человекочитаемой строкой ({@code prerequisite.text}). Отдельной строкой записи оно
     * не едет — в описании черты его нет, это разные колонки.
     */
    private VttgFeatData featData;
    /**
     * Остаток механики, которого форма листа не выражает: варианты повышения «или/или»,
     * смена типа существа, владения-ссылки без ключа справочника. Дублем {@link #featData}
