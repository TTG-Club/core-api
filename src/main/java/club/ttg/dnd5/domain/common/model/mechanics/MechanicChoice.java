...
public class MechanicChoice {
    ...
    /**
     * Ступени количества по уровням; пусто — количество не растёт и задано {@link #count}.
     *
     * <p>Ступень называет итог, а не прибавку: оружейных приёмов у воина три с первого
     * уровня, четыре с четвёртого, пять с десятого. Мастер повышения уровня спрашивает
     * разницу с предыдущей ступенью, а колонка таблицы показывает сам ряд.</p>
     */
    @Schema(description = "Ступени количества по уровням; пусто — количество не растёт",
     ...
     *
     * <p>Ряд по уровням у выбора уже задан ступенями, и колонкой его не набирают второй
     * раз — она собирается из них ({@code CounterTableColumns}). Без ступеней показывать
     * нечего: количество, не зависящее от уровня, ряда не имеет.</p>
     */
    @Schema(description = "Показывать количество колонкой таблицы прогрессии")
    private Boolean showInTable;

    /**
     * Краткая подпись колонки таблицы; пусто — берётся {@link #label}.
     *
     * <p>Своя, потому что подпись выбора — это вопрос игроку («Выберите оружейный
     * приём»), а в шапке таблицы нужно существительное («Приёмы»).</p>
     */
    @Schema(description = "Краткая подпись колонки таблицы", example = "Приёмы")
    private String shortName;

    /**
     * Выбранные заклинания не нужно готовить, и места в числе класса они не занимают —
     * только у выбора заклинания или заговора. То же, что {@link SpellGrant#getAlwaysPrepared()}
     * у выдачи, но для заклинаний, которые называет игрок.
     *
     * <p>Так устроены заклинания «сверх таблицы класса»: «Чудотворец» жреца даёт ещё один
     * заговор, который не входит в колонку «Заговоры», а «Договор гримуара» колдуна — три
     * заговора и два ритуала, всегда подготовленных, пока книга при нём.</p>
     *
     * <p>{@code null} читается как «готовить нужно»: выбранное заклинание считается наравне
     * с книгой.</p>
     */
    @Schema(description = "Выбранные заклинания всегда подготовлены и не занимают место в числе класса")
