...
import club.ttg.dnd5.domain.beastiary.service.BeastSectionService;
...
@RequestMapping("/api/v2/beast-section")
public class BeastSectionController {
    private final BeastSectionService sectionService;
    ...
    @PostMapping("/search")
    public List<BeastSectionShortResponse> search(@RequestParam(name = "query", required = false)
                                              @Valid
                                              @Size(min = 3)
    ...
    @GetMapping("/{url}")
    public BeastSectionDetailResponse getByUrl(@PathVariable String url) {
        return sectionService.findDetailedByUrl(url);
    }

    @GetMapping("/{url}/update")
    public BeastSectionRequest getFormByUrl(@PathVariable String url) {
    ...
    public String create(@RequestBody BeastSectionRequest request) {
    ...
    public String update(@PathVariable String url,
                                      @Valid
                                      @RequestBody BeastSectionRequest request) {
