...
public interface MagicItemRepository extends JpaRepository<MagicItem, String> {
     ...
     *       (зачарованное оружие/доспех/посох).</li>
     * </ul>
     * Адамантиновые шаблоны («Адамантиновое оружие», «Адамантиновый доспех») здесь больше НЕ
     * исключаются: выгрузка раскрывает их в конкретные предметы по связанным немагическим
     * предметам ({@code VttgMagicItemMapper.toVttgVariants}) — иначе эффект адамантина до VTTG
     * не доходит вовсе. Шаблон без связей не экспортируется: это решает сам маппер, чтобы в
     * компендиум не попала запись без характеристик.
     * <p>Применяется ко всем {@code *ForVttgExport}-запросам, чтобы /status и /changes были
     * согласованы.</p>
     */
