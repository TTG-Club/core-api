 ...
 *
 * <p>Заряды ({@link #resource}) уходят в выгрузку блоком {@code uses}: применение
 * тратит заряд раньше количества. Условие применения ({@link #activation}) полем
 * предмета не выгружается — в VTTG «когда действует» решает {@code activation}
 * самого эффекта, который ставит редактор; из условия берётся только признак
 * расхода ({@code consumable}) у «при использовании».</p>
 *
