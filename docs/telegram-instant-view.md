...
Пост в канале — короткий: заголовок, анонс и ссылка «Читать полностью». Полный текст открывается
в Telegram нативно (Instant View), поэтому лимит 4096 символов, хвостовые сообщения и заливка
обложки отдельным фото больше не нужны.
...
3. [TelegramPublisher](../src/main/java/club/ttg/dnd5/domain/article/service/TelegramPublisher.java)
   постит сообщение со ссылкой `https://t.me/iv?url=<адрес страницы>&rhash=<rhash>` и передаёт её же
   в `link_preview_options` — карточка с обложкой и кнопкой Instant View показывается над текстом.
